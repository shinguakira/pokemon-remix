...
export class BattleScene extends Scene {
  ...
  private updateAnimations(deltaTime: number): void {
    ...
    if (this.currentState === BattleState.INTRO_PLAYER_POKEMON) {
      if (
        this.positions.playerPokemon.x <= this.positions.playerPokemon.finalX
      ) {
  ...
  draw(p: P5Instance): void {
    ...
    if (this.sprites.npcPokemon) {
      p.image(
        this.sprites.npcPokemon,
        this.positions.npcPokemon.x,
        this.positions.npcPokemon.y
      );
    }
    this.drawDataBox(
      p,
      this.npcPokemon,
      this.positions.npcDataBox,
      this.sprites.npcDataBox
    );
  ...
  private async handlePlayerAttack(): Promise<void> {
    ...
    this.dialogBox.clearText();
    this.dialogBox.displayText(
      `${this.playerPokemon.name} used ${move.name}!`,
      async () => {
        this.playerPokemon.performAttack(
          this.npcPokemon,
  ...
  private async handleNpcAttack(): Promise<void> {
    ...
    this.dialogBox.clearText();
    this.dialogBox.displayText(
      `The foe's ${this.npcPokemon.name} used ${move.name}!`,
      async () => {
