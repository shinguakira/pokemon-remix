...
export class SettingsMenuScene {
  ...
  // Menu dimensions
  private menuWidth: number = 120;
  private menuX: number = 512 - 130;
  private menuY: number = 10;
  private itemHeight: number = 28;

  // Animation
  private slideOffset: number = 150;
  private targetSlideOffset: number = 0;
  ...
  private drawPokemonSlot(
    ...
    if (isActive) {
      ...
      p.fill(0);
      p.text(
        pokemon.stats.maxHp + " / " + pokemon.stats.maxHp,
        hpBarX,
        hpBarY + 14
      );
    }
